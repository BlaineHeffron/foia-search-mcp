...
function digitalObjectUrl(record: Record<string, unknown>): string | undefined {
  const text = JSON.stringify(record);
  const match = text.match(/https?:\/\/[^"\\]+(?:\.pdf|\.jpg|\.jpeg|\.png|\.tif|\.tiff)/i);
  return match?.[0];
}

function toSearchResult(record: Record<string, unknown>): SearchResult {
  const id =
    firstString(record.naId) ??
...
export async function searchNaraCatalog(params: NaraSearchParams): Promise<SearchResponse> {
  ...
  if (params.available_online ?? true) url.searchParams.set("availableOnline", "true");
...
export async function getNaraRecord(naid: string, base_url: string): Promise<DocumentDetail> {
  ...
  if (text.trimStart().startsWith("<")) {
    return {
      ...
      metadata: {},
      citation_note:
        "NARA returned HTML instead of JSON from the configured API endpoint. Verify this NAID manually in the Catalog.",
    };
  }
  const payload = JSON.parse(text) as unknown;
  const record = asRecords(payload)[0] ?? ({ naId: naid, title: naid } as Record<string, unknown>);
  return {
