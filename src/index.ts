...
export function createServer() {
  ...
  server.tool(
    "search_cia_reading_room",
    "Search CIA FOIA Electronic Reading Room pages for declassified documents. Use for CIA CREST/FOIA records, historical intelligence memos, and weather/plasma/ionosphere terms. This scrapes public search HTML, so verify important hits against source PDFs.",
    {
  ...
  server.tool(
    "get_cia_document",
    "Fetch metadata and source links for one CIA Reading Room document. Use after search_cia_reading_room returns an id, or when you already have a /readingroom/document/ URL. Output includes scan/PDF links when the page exposes them; OCR may be incomplete.",
    {
  ...
  server.tool(
    "search_nara_catalog",
    "Search National Archives Catalog metadata for federal/declassified records. Use for archival record groups, digitized holdings, and official metadata beyond CIA. The Catalog API can be inconsistent; use broader terms and available_online=false if needed.",
    {
  ...
  server.tool(
    "get_nara_record",
    "Fetch one National Archives Catalog record by NAID. Use after search_nara_catalog when you need complete metadata and digitized-object links. Verify scans/OCR at the Catalog URL before citing.",
    {
  ...
  server.tool(
    "search_official_declass_sources",
    "Return official/public-source search entry points suited to a declassified-doc research query. Use when deciding where to search next across CIA, NARA, FRUS, DTIC, GovInfo, and NOAA. This does not search all documents; it gives source-specific search URLs and caveats.",
    {
