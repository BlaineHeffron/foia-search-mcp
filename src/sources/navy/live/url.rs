...
pub(crate) fn canonicalize_official_url(url: &str, base_url: &str) -> String {
    ...
    if url_host_name.eq_ignore_ascii_case("secnav.navy.mil")
        && base_host.eq_ignore_ascii_case("www.secnav.navy.mil")
    {
        return url.replacen("://secnav.navy.mil", "://www.secnav.navy.mil", 1);
    }
    if url_host_name.eq_ignore_ascii_case("www.secnav.navy.mil")
        && base_host.eq_ignore_ascii_case("www.secnav.navy.mil")
...
pub(crate) fn is_allowed_navy_url(url: &str, base_url: &str) -> bool {
    ...
    !path.trim_matches('/').is_empty()
        && (path.starts_with("/foia") || path.starts_with("/ig") || path.starts_with("/navaudsvc"))
}
...
pub(crate) fn source_id_from_url(url: &str) -> String {
    ...
    match (path.is_empty(), query.is_empty()) {
        (true, _) => "foia/readingroom".to_owned(),
        (false, true) => path,
