...
pub(crate) fn canonicalize_official_url(url: &str, base_url: &str) -> String {
    ...
    if url_host_name.eq_ignore_ascii_case("dia.mil")
        && base_host.eq_ignore_ascii_case("www.dia.mil")
    {
        return url.replacen("://dia.mil", "://www.dia.mil", 1);
    }
    if url_host_name.eq_ignore_ascii_case("www.dia.mil")
        && base_host.eq_ignore_ascii_case("www.dia.mil")
...
pub(crate) fn is_allowed_dia_url(url: &str, base_url: &str) -> bool {
...
pub(crate) fn source_id_from_url(url: &str) -> String {
    ...
    match (path.is_empty(), query.is_empty()) {
        (true, _) => "FOIA/FOIA-Electronic-Reading-Room".to_owned(),
        (false, true) => path,
