...
use super::url::{
    absolutize, canonicalize_official_url, document_key, is_allowed_dia_url, source_id_from_url,
};
use super::{dia_citation_note, dia_terms_note, DIA_READING_ROOM_PATH, DIA_SOURCE};

pub(crate) const SOURCE_WARNING: &str = "DIA FOIA Electronic Reading Room results are official DIA FOIA leads. Page-level citations require ingesting the linked PDF and verifying page boundaries, OCR quality, redactions, and any document-origin context before publication.";
...
pub(crate) fn records_from_reading_room_page(
    ...
    for (href, text) in anchors(html) {
        ...
        if !is_allowed_dia_url(&url, base_url) || is_navigation_label(&text) {
            continue;
        }
...
pub(crate) fn record_from_detail_page(
    ...
    let mut attachments = anchors(html)
        ...
        .filter_map(|(href, label)| {
            ...
            if !is_allowed_dia_url(&url, base_url) || !is_likely_asset_link(&url, &label) {
                return None;
            }
    ...
    metadata.insert("listing_origin".to_owned(), "dia_record_page".to_owned());
    ...
    Some(SourceRecord {
        id: format!("{DIA_SOURCE}:{source_id}"),
        document_key: document_key(DIA_SOURCE, &source_id),
        source: DIA_SOURCE,
        source_id,
        title,
        date: None,
        collection: Some("DIA FOIA Electronic Reading Room".to_owned()),
        record_group: Some("dia_foia_electronic_reading_room".to_owned()),
        description: Some(description_for_record(pdf_url.is_some()).to_owned()),
        origin_url: format!(
            "{}{}",
            base_url.trim_end_matches('/'),
            DIA_READING_ROOM_PATH
        ),
        ...
        citation_note: Some(dia_citation_note().to_owned()),
        terms_note: Some(dia_terms_note().to_owned()),
    })
}
...
fn looks_like_reading_room_lead(url: &str, label: &str) -> bool {
    ...
    lower_url.contains("/foia/foia-electronic-reading-room/")
        || lower_url.contains("/fileid/")
        || lower_url.ends_with(".pdf")
        || lower_label.contains("pdf")
        || lower_label.contains("report")
        || lower_label.contains("intelligence")
}

fn is_navigation_label(label: &str) -> bool {
    matches!(
        label.trim().to_ascii_lowercase().as_str(),
        "" | "home" | "foia" | "search" | "clear" | "contact us" | "privacy"
    )
}
...
fn base_metadata(url: &str, source_id: &str, title: &str) -> SourceMetadata {
    ...
    metadata.insert("official_source".to_owned(), "www.dia.mil".to_owned());
    metadata
}
...
fn title_from_heading(heading: &str) -> String {
    heading
        .trim()
        .trim_start_matches("Defense Intelligence Agency >")
        .trim()
        .to_owned()
}
...
fn description_for_record(has_pdf: bool) -> &'static str {
    if has_pdf {
        "Official DIA FOIA Electronic Reading Room lead with PDF asset metadata."
    } else {
        "Official DIA FOIA Electronic Reading Room lead; inspect linked assets before citation."
    }
}
