...
pub(crate) fn release_date_from_index(html: &str) -> Option<String> {
    let upper = html.to_ascii_uppercase();
    let marker = "CLEARED FOR RELEASE -";
    let index = upper.find(marker)?;
    let tail = html.get(index + marker.len()..)?;
    let date = tail.lines().next()?.trim();
    if date.is_empty() {
        None
    } else {
        Some(date.to_owned())
