...
pub(crate) fn asset_from_link(url: &str, label: &str) -> SourceAsset {
    ...
    let (role, mime_type) =
        if lower.ends_with(".pdf") || lower.contains(".pdf?") || label_lower.ends_with(".pdf") {
            ...
            || lower.ends_with(".htm")
            || lower.contains("/home/publicrecords/")
        {
...
pub(crate) fn is_likely_asset_link(url: &str, label: &str) -> bool {
    let lower = url.to_ascii_lowercase();
    let label_lower = label.to_ascii_lowercase();
    lower.contains("/home/doccontent/")
        || lower.ends_with(".pdf")
        || lower.contains(".pdf?")
        || lower.ends_with(".zip")
        || lower.contains(".zip?")
        || lower.ends_with(".txt")
        || lower.contains(".txt?")
        || label_lower.ends_with(".pdf")
        || label_lower.ends_with(".zip")
}

pub(crate) fn is_direct_download_url(url: &str) -> bool {
    let lower = url.to_ascii_lowercase();
    lower.contains("/home/doccontent/")
        || lower.ends_with(".pdf")
        || lower.contains(".pdf?")
