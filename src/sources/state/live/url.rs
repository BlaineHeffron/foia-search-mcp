...
pub(crate) fn canonicalize_official_url(url: &str, base_url: &str) -> String {
    ...
    if url_host_name.eq_ignore_ascii_case("www.foia.state.gov")
        && base_host.eq_ignore_ascii_case("foia.state.gov")
    {
        return url.replacen("://www.foia.state.gov", "://foia.state.gov", 1);
    }
    if url_host_name.eq_ignore_ascii_case("foia.state.gov")
        && base_host.eq_ignore_ascii_case("foia.state.gov")
...
pub(crate) fn is_allowed_state_url(url: &str, base_url: &str) -> bool {
...
pub(crate) fn source_id_from_url(url: &str) -> String {
    ...
    match (path.is_empty(), query.is_empty()) {
        (true, _) => "Search/Results.aspx".to_owned(),
        (false, true) => path,
...
pub(crate) fn percent_encode_query(value: &str) -> String {
    percent_encode_component(value, true)
}

fn percent_encode_path_segment(value: &str) -> String {
    percent_encode_component(value, false)
}

fn percent_encode_component(value: &str, plus_for_space: bool) -> String {
    ...
    for byte in value.bytes() {
        match byte {
            ...
            b' ' if plus_for_space => encoded.push('+'),
