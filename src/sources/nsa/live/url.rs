...
pub(crate) fn canonicalize_official_url(url: &str, base_url: &str) -> String {
    ...
    if url_host_name.eq_ignore_ascii_case("www.nsa.gov")
        && base_host.eq_ignore_ascii_case("www.nsa.gov")
...
pub(crate) fn is_allowed_nsa_url(url: &str, base_url: &str) -> bool {
    ...
    !path.trim_matches('/').is_empty()
        && (path.starts_with("/Helpful-Links/NSA-FOIA/")
            || path.starts_with("/Portals/75/documents/")
            || path.starts_with("/portals/75/documents/"))
}
...
pub(crate) fn source_id_from_url(url: &str) -> String {
    ...
    if path.is_empty() {
        "Helpful-Links/NSA-FOIA/Reading-Room".to_owned()
    } else {
        path
    }
}
