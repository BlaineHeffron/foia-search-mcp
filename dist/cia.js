...
function makeCursor(page) {
    return Buffer.from(String(page), "utf8").toString("base64url");
}
export function parseCiaSearch(html, base_url, query, page) {
    ...
    const hasNext = $("a[rel='next'], .pager-next a, a:contains('next'), a:contains('Next')").length > 0 ||
        results.length > 0;
    return {
        query,
...
export async function searchCiaReadingRoom(params) {
...
export async function getCiaDocument(id_or_url, base_url) {
    ...
    if (!url.includes("/readingroom/document/")) {
        throw new SourceError("CIA document lookup expects a Reading Room document id or /readingroom/document/ URL.", "cia", undefined, "Pass ids like cia-rdp68r00530a000200110020-2.");
    }
